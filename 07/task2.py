...
class Dir(File):
	...
	def find_directories_greater_than_size(self, size, matching = []):
		dirs = [file for file in self.files if isinstance(file, Dir)]
		for dir in dirs:
			if dir.size > size:
				matching = [dir] + dir.find_directories_greater_than_size(size, matching)
			else:
				matching = dir.find_directories_greater_than_size(size, matching)
		return matching
...
max_filesystem = 70000000
min_required = 30000000
to_delete = root.size - (max_filesystem - min_required)

dirs = sorted(root.find_directories_greater_than_size(to_delete), key=lambda x: x.size)
answer = dirs[0].size
print(answer)
